...
public class LocatorConfiguration extends Locator {
    static protected Configuration config;
    static { config = Configuration.create(RootContext.getInitialisationContext(), "com.sphenon.basics.locating.locators.LocatorConfiguration"); };

    public LocatorConfiguration (CallContext context, String text_locator_value, Locator sub_locator, String locator_class_parameter_string) {
    ...
    protected LocatorParserState[] getParserStates(CallContext context) {
        if (locator_parser_state == null) {
            locator_parser_state = new LocatorParserState[] {
                new LocatorParserState(context, "name", "name::String:1", false, true, String.class),
                new LocatorParserState(context, "default", "default::String:2", false, true, Object.class),
    ...
    protected Object retrieveLocalTarget(CallContext context) throws InvalidLocator {
        ...
        Object value = Configuration.get(context, name, (String) null, (String) null);

        if (value == null && steps.length > 1) {
            value = steps[1].getValue(context);
        }

        if (value == null) {
            InvalidLocator.createAndThrow(context, "While resolving configuration locator '%(locator)', no property named '%(name)' was found within configuration, and no default value provided", "locator", this.getTextLocatorValue(context), "name", name);
